...
'INSTANT VB NOTE: The field BUZZERTIME was renamed since Visual Basic does not allow fields to have the same name as other class members:
			...
			Public Property StartFreq() As UShort
                ...
                Return CUShort(STRATFREI >> 8 Or STRATFREI << 8)

            End Get
				Set(ByVal value As UShort)
                STRATFREI = CUShort(value >> 8 Or value << 8) '大小端转换
            End Set
			End Property
			Public Property StartFreqde() As UShort
				Get
                Return CUShort(STRATFRED >> 8 Or STRATFRED << 8)
            End Get
				Set(ByVal value As UShort)
                STRATFRED = CUShort(value >> 8 Or value << 8) '大小端转换
            End Set
			End Property
			Public Property Stepfreq() As UShort
				Get
                Return CUShort(STEPFRE >> 8 Or STEPFRE << 8)
            End Get
				Set(ByVal value As UShort)
                STEPFRE = CUShort(value >> 8 Or value << 8) '大小端转换
            End Set
			End Property
		End Structure
