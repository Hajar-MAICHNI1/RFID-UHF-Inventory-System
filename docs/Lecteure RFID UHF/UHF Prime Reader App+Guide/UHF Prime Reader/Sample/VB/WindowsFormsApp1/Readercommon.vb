...
Namespace WindowsFormsApp1
	...
	Friend Class API
		...
		<DllImport("UHFPrimeReader.dll")>
		Public Shared Function SetDevicePara_J(ByVal hComm As IntPtr, ByVal DEVICEARRD As Byte, ByVal RFIDPRO As Byte, ByVal WORKMODE As Byte, ByVal [INTERFACE] As Byte, ByVal BAUDRATE As Byte, ByVal WGSET As Byte, ByVal ANT As Byte, ByVal REGION As Byte, ByVal STRATFREI1 As Byte, ByVal STRATFREI2 As Byte, ByVal STRATFRED1 As Byte, ByVal STRATFRED2 As Byte, ByVal STEPFRE1 As Byte, ByVal STEPFRE2 As Byte, ByVal CN As Byte, ByVal RFIDPOWER As Byte, ByVal INVENTORYAREA As Byte, ByVal QVALUE As Byte, ByVal SESSION As Byte, ByVal ACSADDR As Byte, ByVal ACSDATALEN As Byte, ByVal FILTERTIME As Byte, ByVal TRIGGLETIME As Byte, ByVal BUZZERTIME As Byte, ByVal INTENERLTIME As Byte) As Integer
		End Function
...
'ORIGINAL LINE: volatile int m_iState = 0;
		...
		Public Sub SetDevicePara_J(ByVal info As Devicepara)
			If m_iState = 0 Then
				Throw New InvalidOperationException("Reader is not open")
			End If
			'DegbugPrint("Start GetVer");
			Dim state As Integer = API.SetDevicePara_J(m_handler, info.Addr, info.Protocol, info.Workmode, info.port, info.Baud, info.wieggand, info.Ant, info.Region, CByte(CInt(info.StartFreq) >> 8), CByte(info.StartFreq), CByte(CInt(info.StartFreqde) >> 8), CByte(info.StartFreqde), CByte(CInt(info.Stepfreq) >> 8), CByte(info.Stepfreq), info.Channel, info.Power, info.Area, info.Q, info.Session, info.Startaddr, info.DataLen, info.Filtertime, info.Triggletime, info.Buzzertime, info.IntenelTime)
			'DegbugPrint("End GetVer");
			If state = ReaderException.ERROR_SUCCESS Then
				Return
			End If
